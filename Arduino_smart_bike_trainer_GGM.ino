/*
  ...
  By default, SIM mode is disabled by setting OVERRIDE = true which lets you manually control the resistance by changing AMPS
...
float ftpWatts = 120;       // base FTP to use for SIM mode calculations  
float losses = 1.16;         // losses multiplier, account for losses in drive train, rectifier, charge controller, etc
...
boolean OVERRIDE = true; // set to false to enable SIM mode, set to true to manually control resistance by adjusting AMPs on the DPSxxxx
boolean SIMMODE = false; // used to track if in SIM mode or not
     ...
     while (central.connected()) {
      ...
      if (fitnessMachineControlPointCharacteristic.written()) {
          ...
          switch(fmcpData.values.OPCODE) {
            ...
            case fmcpSetIndoorBikeSimulationParameters: {
              ...
              else
              {
                ...
                delay(1000); // TODO: use a timer to check last write instead of this delay
        ...
        {        
          ...
          // if the watts are a off from what was set previously, adjust to match
          currentTime = millis();
          if (SIMMODE) {
            if (!OVERRIDE) {
                // wait 60 seconds after starting, then if user manually changed DPSxxxx to 20 watts or more higher (ie: cranked up the amps by hand), turn on override to they can manually control
