/*
  ...
  along with a DPSxxxx charge controller to adjust the actual resistance while charging batteries with a pedal generator

  ArduinoBLE and ModbusMaster libraries are used for BLE communication and talking to the DPSXXXX.

  Training mode works amazingly well, SIM mode uses grade and RPM/cadence to adjust resistance.  
...
float Watts = 0.0;
float Amps = 0.0;
float Volts = 14.6;  // don't really need to set, we'll read it from the DPSXXXX anyway
...
void setup() {
  ...
  // Hall effect sensor
  pinMode(hallInterruptPin, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(hallInterruptPin), calcRPM, RISING);  
  ...
  if (central) {
     ...
     while (central.connected()) {
      if (fitnessMachineControlPointCharacteristic.written()) {
          ...
          ftmsControlPointBuf[2] =  0x02; // OpCode not supported for now, changed below if appropriate
        ...
        int s = round(RPM*0.14587); // speed in m/s
        indoorBikeDataBuf[2] = s & 0xFF; // Instantaneous Speed, uint16
        indoorBikeDataBuf[3] = (s >> 8) & 0xFF;
        
        int instantaneous_cadence = round(RPM*2);  // RPM times 2, because... Zwift!
        ...
        {        
          ...
          instantaneous_power = round(((float)instantaneous_power)*losses); // account for losses in drive train, bridge rectifier and charge controller
...
int setTrainerResistance(float wind_speed, float grade, float crr, float cw) {
  float rpmFactor = 1.0;
  // calculate RPM factor, leave it at 1.0 if cadence is 80, make it harder if higher, easier if lower
  if (RPM != 80.0)
  {
    rpmFactor = (((RPM-80.0)*simCadenceMultiplier)/100.0) + 1.0;
  }
  // use grade from Zwift along with cadence to adjust trainer resistance 
  return  (int)(((ftpWatts * (grade/10)) + ftpWatts) * rpmFactor);
}
